...
const DATA_URL = 'https://raw.githubusercontent.com/uber-web/kepler.gl-data/master/';
const MAP_CONFIG_URL = `${DATA_URL}samples.json?nocache=${new Date().getTime()}`;
                ...
                if (error) {
                    ...
                    dispatch(
                        loadRemoteResourceError(
                            {
                                status,
                                message: `${responseText} - ${options.id} (${configUrl})`
                            },
                            configUrl
                        )
                    );
